  ...
  <div v-if="data" class="product">
        ...
        <div class="product__rating">
          <div class="product__rating-line">
            <div class="product__stars">
              ...
              <StarPool v-for="star in MAX_NUMBER_OF_STARS - rating" :key="`empty-${star}`" />
            </div>
            <span class="product__reviews-count">{{ data.rating.count }} customer review</span>
          </div>
    ...
    <div class="product__tabs">
      <div class="product__tab-headers">
        ...
        >
          {{ tab.label }}<span v-if="tab.id === 'reviews'">({{ data.rating.count }})</span>
        </h3>
      </div>

      <div class="product__tab-content">
        <ItemPageDescription v-if="currentTab === 'description'" :text="data.description" />
        ...
        <keep-alive>
          <ItemPageReviews v-if="currentTab === 'reviews'" :count="data.rating.count" />
        </keep-alive>
      </div>
    </div>
...
<script lang="ts" setup>
  ...
  const rating = computed(() => (data.value?.rating ? Math.round(data.value.rating.rate) : 0))
  ...
  const autoChangePhoto = () => {
    currentPhotoIndex.value = (currentPhotoIndex.value + 1) % arrayOfPhotos.length
