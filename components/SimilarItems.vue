  ...
  <div class="similar-items">
    ...
    <div class="catalog-latest">
      <template v-if="isLoading">
        <LoadingSkeletonCard v-for="n in REQUIRED_NUMBER_OF_CARDS_SIMILAR" :key="`skeleton-${n}`" />
      </template>
      <template v-else>
        <ItemCard v-for="cards in displayedItems" :key="cards.id" :product="cards" class="mobile" />
      </template>
    </div>
  </div>
</template>
